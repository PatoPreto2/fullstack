...
<p>Sua fatura deste mês venceu hoje e já está paga de acordo com seu plano. Qualquer dúvida estamos a disposição.</p>"
            ...
            $body = $view->render("mail", [
                "subject" => $subject,
                "message" => "<h3>Presado {$user->first_name}!</h3>
<p>Não conseguimos cobrar seu cartão referênte a fatura deste mês para sua assinatura LuverApp.
 Precisamos que você veja isso.</p><p>Acesse sua conta para atualizar seus dados de pagamento,
  você pode cadastrar outro cartão.</p><p>Se não fizer nada agora uma nova tentativa de 
  cobrança será feita em 3 dias. Se não der certo, sua assinatura será cancelada :/</p>
 ...
 */
$chargeDays = $subscription->find("pay_status = :status AND next_due + INTERVAL 3 DAY 
= date(NOW()) AND last_charge != DATE(NOW())",
    "status=active")->fetch(true);
...
<p>Tentamos efetuar mais uma cobrança para sua assinatura {$plan->name} hoje, mas sem sucesso.</p>
<p>Como essa já é uma segunda tentativa, infelismente sua assinatura foi cancelada. Mas calma, 
você pode assinar novamente a qualquer momento.</p><p>Continue controlando com os recursos FREE, 
e assim que quiser basta assinar novamente e voltar de onde parou :)</p>"
            ]);
