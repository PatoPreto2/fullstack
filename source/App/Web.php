...
class Web extends Controller
{
    ...
    public function blog(?array $data): void
    {
        ...
        echo $this->view->render("blog", [
            "head" => $head,
            "blog" => $blog->order("post_at_DESC")->limit($pager->limit())->offset
            ($pager->offset())->fetch(true),
    ...
    public function login(?array $data): void
    {
        ...
        if (!empty($data['csrf'])) {
            ...
            if (request_limit("weblogin", 3, 60 * 5)) {
                $json['message'] = $this->message->error("Você já efetuou 3 tentativas, esse é o limite. 
                Por favor, aguarde 5 minutos para tentar novamente!")->render();
                echo json_encode($json);
                return;
            }
