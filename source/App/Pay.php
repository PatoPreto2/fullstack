...
class Pay extends Controller
{
    ...
    public function create(array $data): void
    {
        ...
        if (request_limit("paycreate", 3, 60 * 5)) {
            $json["message"] = $this->message->warning("Desculpe {$user->first_name}, 
            mas por segurança aguarde pelo menos 5 minutos para tentar outro cartão.")->render();
        ...
        if ($checkSubscribe) {
            $json["message"] = $this->message->warning("Você já tem uma assinatura ativa {$user->first_name}. 
            Não é necessário assinar o {$plan->name} mais de uma vez.")->render();
            echo json_encode($json);
            return;
        }
        ...
        $this->message->success("Bem-vindo(a) ao {$plan->plan} {$user->first_name}. 
        Sua assinatura está ativa e você já pode controlar. Confira os detalhes...")->flash();
    ...
    public function update(array $data): void
    {
        ...
        if ($subscribe->status == "past_due") {
            ...
            if ($transaction) {
                ...
                $subscribe->next_due = date("Y-m-d", strtotime($subscribe->next_due .
